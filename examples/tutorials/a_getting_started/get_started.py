...
#     - **Regular grids**: Define a resolution (and refinement=None). A medium resolution of 50x50x50, for example, results in 125,000 voxels. Model voxels are prisms, not cubes, so resolution can differ from extent. Avoid exceeding 100 cells in each direction (1,000,000 voxels) to prevent high computational costs.
#     - **Octrees**: Define a level of refinement (and resolution=None). Higher refinement levels increase computational costs.
# 
# > **Note on choice of modeling grids:** Which type of grid is used depends on the use case. Note that as of the current version of GemPy 3, the rendering of surfaces uses dual-contouring, which is based on octrees. So even if you choose regular grids, octree-based computing will be executed additionally in order to render the surfaces in 3D.
#%%
...
#%% md
# As you can see, by default, there is one element called 'surface 1' and no faults. However, by default, GemPy actually not only starts off with this 'surface 1' but also a 'basement' unit which is always present. We can see this using the following function:
#%%
...
#%%
# 3D visualization:
gpv.plot_3d(geo_model, show_surfaces=True, image=True, show_lith=False)
#%% md
...
#%%
gpv.plot_3d(geo_model, show_surfaces=True, image=True, show_topography = True, show_lith=False)
#%% md
...
#%%
gpv.plot_3d(geo_model, show_surfaces=True, image=True, show_topography = True)
#%% md
...
# 
# While this topography is random, GemPy also has the capability to include real topography files and arrays via the functions `set_topography_from_file` and `set_topography_from_arrays`.
#%% md
...
# - `geo_model.solutions.raw_arrays`: An object containing numerous arrays that define various parts of the model. Of particular importance are the lithology block (`lith_block`), the fault block (`fault_block`), and the scalar field matrix (`scalar_field_matrix`).
# 
# ### Mesh Solutions
# 
