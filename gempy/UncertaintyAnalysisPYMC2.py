...
class Plane:
    def __init__(self, group_id, geo_data):
        self.group_id = group_id
        self.geo_data = geo_data

        # create dataframe bool filters for convenience
        self.fol_f = self.geo_data.foliations["group_id"] == self.group_id
        self.interf_f = self.geo_data.interfaces["group_id"] == self.group_id

        # get indices for both foliations and interfaces
        self.interf_i = self.geo_data.interfaces[self.interf_f].index
        self.fol_i = self.geo_data.foliations[self.fol_f].index[0]

        # normal
        self.normal = self.get_normal()
        # centroid
        self.centroid = [float(self.geo_data.foliations[self.fol_f]["X"]),
                         float(self.geo_data.foliations[self.fol_f]["Y"]),
                         float(self.geo_data.foliations[self.fol_f]["Z"])]
    ...
    def interf_recalc(self, dip):
        ...
        self.geo_data.foliations.set_value(self.fol_i, "dip", dip)
        # get azimuth
        az = float(self.geo_data.foliations.iloc[self.fol_i]["azimuth"])
        # set polarity according to dip
        if -90 < dip < 90:
            polarity = 1
        else:
            polarity = -1
        self.geo_data.foliations.set_value(self.fol_i, "polarity", polarity)
        # modify gradient
        self.geo_data.foliations.set_value(self.fol_i, "G_x",
                                           np.sin(np.deg2rad(dip)) * np.sin(np.deg2rad(az)) * polarity)
        self.geo_data.foliations.set_value(self.fol_i, "G_y",
                                           np.sin(np.deg2rad(dip)) * np.cos(np.deg2rad(az)) * polarity)
        self.geo_data.foliations.set_value(self.fol_i, "G_z", np.cos(np.deg2rad(dip)) * polarity)
        ...
        for i, row in self.geo_data.interfaces[self.interf_f].iterrows():
    ...
    def get_normal(self):
        ...
        normal = [float(self.geo_data.foliations.iloc[self.fol_i]["G_x"]),
                  float(self.geo_data.foliations.iloc[self.fol_i]["G_y"]),
                  float(self.geo_data.foliations.iloc[self.fol_i]["G_z"])]
        return normal
