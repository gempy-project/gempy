...
def plot_add_surface_points(func):
    def pasp(*args, **kwargs):
        surface_points, idx = func(*args)

        if 'plot_object' in kwargs:
            if isinstance(kwargs['plot_object'], GemPyvtkInteract):
                kwargs['plot_object'].render_add_surface_points(idx)
            else:
...
def plot_delete_surface_points(func):
    def pdsp(*args, **kwargs):
        surface_points = func(*args)

        if 'plot_object' in kwargs:
            if isinstance(kwargs['plot_object'], GemPyvtkInteract):
                kwargs['plot_object'].render_delete_surface_points(args[1])
            else:
                raise AttributeError('plot_object must be one GemPy compatible plot')
        return surface_points
    return pdsp
...
def plot_add_orientation(func):
    def pao(*args, **kwargs):
        orientation, idx = func(*args)

        if 'plot_object' in kwargs:
            if isinstance(kwargs['plot_object'], GemPyvtkInteract):
                kwargs['plot_object'].render_add_orientation(idx)
            else:
                raise AttributeError('plot_object must be one GemPy compatible plot')
        return orientation
    return pao
...
def plot_delete_orientations(func):
    def pdo(*args, **kwargs):
        orientations = func(*args)

        if 'plot_object' in kwargs:
            if isinstance(kwargs['plot_object'], GemPyvtkInteract):
                kwargs['plot_object'].render_delete_orientations(args[1])
            else:
                raise AttributeError('plot_object must be one GemPy compatible plot')
        return orientations
    return pdo
